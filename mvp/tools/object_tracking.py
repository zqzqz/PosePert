...
class Ab3dmotTracker():
    """AB3DMOT wrapper used by the scenario attacker and the CARLA closed-loop stack.

    Kept alongside the EKF Tracker above: the scenario pipeline tracks detections with
    AB3DMOT, while sync_handler still uses the simpler EKF tracker.
    """
    ...
    def update(self, t, detections, info):
        ...
        result = self.tracker.update(t, tracker_input)
        bboxes = result[:, :7]
        bbox_ids = result[:, 7].astype(int)
        bbox_features = result[:, 8:]
        return bboxes, bbox_ids, bbox_features
