...
class AB3DMOT(object):  # A baseline of 3D multi-object tracking
    def __init__(
        self, max_age=10, min_hits=3, info_dim=1
    ...
    def update(self, t, dets_all, z_up=False):
        ...
        Params:
          dets_all: dict
                dets - a numpy array of detections in the format [[x,y,z,l,w,h,theta],...]
        ...
        for t_ID in reversed(to_del):
            self.trackers.pop(t_ID)
        ...
        for t_ID, trk in enumerate(self.trackers):
            if t_ID not in unmatched_trks:
                d = matched[np.where(matched[:, 1] == t_ID)[0], 0]  # a list of index
        ...
        return np.empty((0, 10 + self.info_dim))